...
#include <linux/switchscheduler.h> //phase 2
...
extern int phase2switchflag; //phase 2
...
int user_process_count[10]; //phase 2
...
need_resched_back:
	...
	//phase 2
	if(phase2switchflag){ //switch to default scheduler
...
repeat_schedule:
	...
	//phase 2 fair-share scheduler
		...
		if (unlikely(!c)) {
			...
			memset(user_process_count, 0, sizeof(user_process_count));
			
			spin_unlock_irq(&runqueue_lock);
			read_lock(&tasklist_lock);
			
			for_each_task(p) {
				if(p->uid>=1000 && p->uid<=1009) {
					user_process_count[p->uid%10]++; 
				}
			}
			
			for_each_task(p) {
				int process_multiplication = 1;
				
				int i = 0;
				while(i < 10){
					if(user_process_count[i] != 0){
						process_multiplication *= user_process_count[i];
					}
					i++;
				}
				
				process_multiplication *= 6;
				
				if(p->uid>=1000 && p->uid<=1009) {
					p->counter = process_multiplication / user_process_count[p->uid%10];
				}
				else {
					p->counter = (p->counter >> 1) + NICE_TO_TICKS(p->nice);
				}
